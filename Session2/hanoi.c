...
void Hanoi(int n, int A, int B, int C) {
  ...
  Hanoi(n-1, A, C, B);      //Take top n-1 disks from A, put them on C, use B as auxiliary


  p[B] = p[B] * 10 + p[A] % 10;  //Take top disk from A (afther you took n - 1 on C), put it in B
  p[A] = p[A] / 10;
  ...
  Hanoi(n-1, C, B, A);    //Take top n-1 disks from C (Where you put the previous n-1 from A), put them in B, use A as auxiliary
}
